...
{	
	...
	while(1)
	{
		...
		if (strlen(uart_buf) > 6 ) { //这里是只要了前6个数，但也出现了一种情况，就是串口发送的数据总长度小于6，也被筛出去了，所以得改成>6
    		printf("Error: Invalid data length.\n");
    		continue; // 数据长度不符合预期，跳过当前循环
		}
		if (!isdigit(uart_buf[0]) || !isdigit(uart_buf[1]) || !isdigit(uart_buf[2]) || !isdigit(uart_buf[3]) || !isdigit(uart_buf[4]) ) { //这里只检测前5位是否为数字，如果第6位不为数字就说明光照强度很低，要单独拉出来处理
		...
		humi = ((uart_buf[2] - '0') * 10) + (uart_buf[3] - '0');
		
		if (!isdigit(uart_buf[5])) { //如果第6位数据不为数字
			light = (uart_buf[4] - '0');
		}else{
			light = ((uart_buf[4] - '0') * 10) + (uart_buf[5] - '0');
		}
		...
		case 2://状态2发送LIGH数据
			memset(buf ,0 ,sizeof(buf));
			OneNet_SendData(buf,"ligh",light);
			sleep(1);
			printf("Send LIGH: %d \n", light); // 打印
			onenet_send_state = 3;
		case 3://状态3发送数据分割线
