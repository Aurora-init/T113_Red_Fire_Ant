...
int gps_analyse(char *buff, GNGGA *gps_data)
{
    ...
    sscanf(ptr, "$GNGGA,%d.000,%d.%d,N,%d.%d,E,%d,%d,%d.%d,%d.%d,M,%d.%d,M,,*4%c",
           &(gps_data_buf->time),                			/* gps定位时间 */
           &(gps_data_buf->latitude),            			/* 纬度 */
           &(gps_data_buf->latitude_x),            			/* 纬度小数 */
           &(gps_data_buf->longitude),           			/* 经度 */
           &(gps_data_buf->longitude_x),           			/* 经度小数 */
           &(gps_data_buf->GPS_quilty), 		 			/* GPS质量 */
           &(gps_data_buf->Number_of_Satellites_in_Use),    /* 卫星数量 */
           &(gps_data_buf->HDOP),							/* HDOP */
           &(gps_data_buf->HDOP_x),							/* HDOP小数 */
           &(gps_data_buf->Altitude),	 			        /* 海拔 */
           &(gps_data_buf->Altitude_x),	 			        /* 海拔小数 */
           &(gps_data_buf->Geoidal_Separation), 		    /* 大地水准位间距 */
           &(gps_data_buf->Geoidal_Separation_x), 		    /* 大地水准位间距 */
           &(gps_data_buf->str)); 		    				/* 校验 */
    
    sscanf(ptr,"%s",str);
	
	/*例子:2311.77676传进去，将被分为longitude=2311，longitude_x=77676，第一部分23.00000，第二部分11.0000*/
